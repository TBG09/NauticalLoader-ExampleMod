...
public class Arguments {
    ...
    public static void handleArguments(String[] args, Scanner scanner, IOHandler.IO io, Window window, long totalMemoryMB, boolean runningOnTermux, CountDownLatch latch) throws InterruptedException {
        ...
        if (hasWindow) {
            if (totalMemoryMB >= 16) {
                if (!runningOnTermux) {
                    Logger.info("Learn.Core.Arguments", "Learn.Window Launching!");
                    window.StartWindow("Just Learn java 4.1.1(Test Learn.Window)");
                    Logger.info("Learn.Core.Arguments", "Not a termux environment, continuing with window launch.");
                    latch.await(); // Wait until the window is closed
                } else {
                    Logger.error("Learn.Core.Arguments", "Learn.Window launch canceled due to being run in a Termux environment.");
                    System.exit(1);
                }
            } else {
                Logger.error("Learn.Core.Arguments", "Memory amount does not meet requirements to launch window.");
        ...
        if (hasHexadecimal) {
            ...
            if (io.isFileEmpty(FileObjectHexData)) {
                Logger.error("Learn.Core.Arguments - Learn.HexadecimalHandler", "File is empty, unable to retrieve any data.");
                System.exit(1);
        ...
        if (hasCredits) {
            ...
            System.out.println("96% coded by Bluelist");
            System.out.println("4% Code given by ChatGPT and thank you to ChatGPT for helping me :D");
            System.exit(0);
        }

        if (hasChangelog) {
            System.out.println("Changes in version " + PublicVariables.VersionNum + ":");
                    System.out.println("Optimized some code:D");
            System.exit(0);
        }

        if (hasSnake) {
            Logger.info("Learn.Core.Arguments", "Starting method Learn.SnakeGame.");
